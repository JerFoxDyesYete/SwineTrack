...
const uint32_t PERIODIC_SNAPSHOT_COOLDOWN_MS = 5UL * 60UL * 1000UL;
const uint32_t DETECTION_SNAPSHOT_COOLDOWN_MS = 1UL * 60UL * 1000UL;
...
void loop() {
  ...
  // --- alert snapshot (full thermal grid), with separate cooldowns ---
  static uint32_t nextPeriodicSnapshotAt = 0;
  static uint32_t detectionCooldownUntil = 0;
  static bool snapshotTimersInitialized = false;
  if (!snapshotTimersInitialized) {
    nextPeriodicSnapshotAt = now + PERIODIC_SNAPSHOT_COOLDOWN_MS;
  ...
  bool detectionDue = gCloudOnline && (now >= detectionCooldownUntil) && detectionTriggered;
  bool periodicDue = gCloudOnline && (now >= nextPeriodicSnapshotAt);

  auto sendSnapshot = [&](bool triggeredByPeriodic, bool detectionEligible) -> bool {
    std::vector<uint8_t> jpeg;
    ...
    const bool triggeredByPeriodicOnly = triggeredByPeriodic && !detectionEligible;
  ...
  if (detectionDue) {
    if (sendSnapshot(false, true)) {
      detectionCooldownUntil = now + DETECTION_SNAPSHOT_COOLDOWN_MS;
      detectionDue = false;  // keep periodic reason clean if also due
    }
  }

  if (periodicDue) {
    if (sendSnapshot(true, detectionDue)) {
      nextPeriodicSnapshotAt = now + PERIODIC_SNAPSHOT_COOLDOWN_MS;
    }
  }
